...
export default {
  ...
  methods: {
    ...
    async selectItem (item) {
      ...
      let choix = [
        {id: 4, title: this.IntlString('APP_PHONE_DELETE'), icons: 'fa-trash', color: 'orange'},
        {id: 5, title: this.IntlString('APP_PHONE_DELETE_ALL'), icons: 'fa-trash', color: 'red'},
        {id: 6, title: this.IntlString('CANCEL'), icons: 'fa-undo'}
      ]
      if (isValid === true) {
        if (this.getContact(numero) === undefined) {
          choix = [
            ...
            {id: 3, title: this.IntlString('APP_PHONE_ADD'), icons: 'fa-user-plus'},
            ...choix]
        } else {
          choix = [
            {id: 0, title: this.IntlString('APP_PHONE_CALL'), icons: 'fa-phone'},
      ...
      switch (rep.id) {
        case 0:
          this.startCall({ numero: item.num })
          break
        case 1:
          this.startCall({ numero: '#' + item.num })
          break
        case 2:
          this.$router.push({ name: 'messages.view', params: {number: item.num} })
          break
        case 3:
          this.$router.push({ name: 'contacts.view', params: {id: -1, number: item.num} })
          break
        case 4:
