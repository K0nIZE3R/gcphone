<template>
  <div class="phone_content">
    ...
    <div class="tweets-wrapper" ref="elementsDiv">
        <div class="tweet" v-for='(tweet, key) in tweetList'
          v-bind:key="tweet.id"
          v-bind:class="{ select: key === selectMessage}"
          >
          <div class="tweet-img">
            <img :src="tweet.authorIcon || '/html/static/img/default_profile.png'" onerror="this.src='/html/static/img/default_profile.png'" width="48" height="48"/>
          </div>
          <div class="tweet-content">
            <div class="tweet-head">
              <div class="tweet-head-author">{{ tweet.author }}</div>
              <timeago class="tweet-head-time" :since='tweet.time' :auto-update="20"></timeago>
            </div>
...
export default {
  ...
  computed: {
    ...
    tweetList () {
      return this.tweets.sort((b, a) => new Date(a.time) - new Date(b.time))
    }
  },
  watch: {
  },
  methods: {
    ...
    async showOption () {
      ...
      const tweet = this.tweetList[this.selectMessage]
      let optionsChoix = [{
        id: 1,
        title: 'Like / Unlike',
        icons: 'fa-heart'
    ...
    onDown () {
      ...
      if (this.selectMessage === -1) {
        ...
        this.selectMessage = this.selectMessage === this.tweetList.length - 1 ? this.selectMessage : this.selectMessage + 1
      }
      this.scrollIntoViewIfNeeded()
    },
    async onEnter () {
