<template>
  <div class='phone_content content inputText'>
    <template v-if="state === STATES.MENU">
      ...
      <template v-if="isLogin">
        <img :src="twitterAvatarUrl" height="128" width="128" style="align-self: center;" onerror="this.src='/html/static/img/default_profile.png'">
    ...
    <template v-else-if="state === STATES.LOGIN">
      <div class="group inputText" data-type="text" data-maxlength='64' :data-defaultValue="localAccount.username">
          <input type="text" :value="localAccount.username" @change="setLocalAccount($event, 'username')" :placeholder="IntlString('APP_TWITTER_ACCOUNT_USERNAME')">
          <span class="highlight"></span>
          <span class="bar"></span>
      </div>

      <div class="group inputText" data-type="text" data-model='password' data-maxlength='30'>
          <input autocomplete="new-password" type="password" :value="localAccount.password" @change="setLocalAccount($event, 'password')" :placeholder="IntlString('APP_TWITTER_ACCOUNT_PASSWORD')">
          <span class="highlight"></span>
          <span class="bar"></span>
      </div>
    ...
    <template v-else-if="state === STATES.ACCOUNT">
      ...
      <div style="margin-top: 42px; margin-bottom: 42px;" class="group img" data-type="button" @click.stop="onPressChangeAvartar">
        <img :src="twitterAvatarUrl" height="128" width="128" @click.stop="onPressChangeAvartar" onerror="this.src='/html/static/img/default_profile.png'">
    ...
    <template v-else-if="state === STATES.NEW_ACCOUNT">
      ...
      <div class="group inputText" data-type="text" data-maxlength='64' data-defaultValue="">
          <input type="text" :value="localAccount.username" @change="setLocalAccount($event, 'username')" :placeholder="IntlString('APP_TWITTER_NEW_ACCOUNT_USERNAME')">
          <span class="highlight"></span>
          <span class="bar"></span>
      </div>

      <div class="group inputText" data-type="text" data-model='password' data-maxlength='30'>
          <input autocomplete="new-password" type="password" :value="localAccount.password" @change="setLocalAccount($event, 'password')" :placeholder="IntlString('APP_TWITTER_NEW_ACCOUNT_PASSWORD')">
          <span class="highlight"></span>
          <span class="bar"></span>
      </div>
      ...
      <div class="group inputText" data-type="text" data-model='password' data-maxlength='30'>
          <input autocomplete="new-password" type="password" :value="localAccount.passwordConfirm" @change="setLocalAccount($event, 'passwordConfirm')" :placeholder="IntlString('APP_TWITTER_NEW_ACCOUNT_PASSWORD_CONFIRM')">
          <span class="highlight"></span>
          <span class="bar"></span>
      </div>
...
export default {
  ...
  methods: {
    ...
    async setLocalAccountAvartar ($event) {
      try {
        const data = await Modal.CreateTextModal({
          title: 'Avatar URL (not use a real photo/selfie)',
          text: this.twitterAvatarUrl || ''
        })
    ...
    async changePassword (value) {
      try {
        const password1 = await Modal.CreateTextModal({limit: 30, title: 'Enter new password'})
        if (password1.text === '') return
        const password2 = await Modal.CreateTextModal({limit: 30, title: 'Enter new password again'})
