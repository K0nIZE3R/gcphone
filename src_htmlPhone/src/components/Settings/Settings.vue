...
export default {
  ...
  computed: {
    ...mapGetters(['IntlString', 'useMouse', 'myPhoneNumber', 'backgroundLabel', 'coqueLabel', 'ringtoneLabel', 'zoom', 'config', 'volume', 'availableLanguages']),
    paramList () {
      ...
      return [
        ...
        {
          icons: 'fa-bell',
          title: this.IntlString('APP_CONFIG_RINGTONE'),
        ...
        },
        {
          ...
          value: this.useMouse && 'Enabled' || 'Disabled',
  ...
  methods: {
    ...mapActions(['getIntlString', 'setZoon', 'setBackground', 'setCoque', 'setRingtone', 'setVolume', 'setMouseSupport']),
    ...
    async onChangeBackground (param, data) {
      ...
      if (val === 'URL') {
        this.ignoreControls = true
        Modal.CreateTextModal({title: 'Background URL'}).then(valueText => {
    ...
    onChangeRingtone: function (param, data) {
      this.setRingtone({
        label: data.title,
        value: data.value
      })
